...
def extract_zip_to_temp():
    ...
    zip_path = os.path.join(sys._MEIPASS, ZIP_FILE_NAME) if getattr(sys, 'frozen', False) else ZIP_FILE_NAME
...
# Funzione per ottenere il refresh rate più alto per la risoluzione 1920x1080
def get_highest_refresh_rate_for_resolution(target_width, target_height):
    device_name = win32api.EnumDisplayDevices(None, 0).DeviceName
    best_mode = None
    max_refresh_rate = 0
    ...
    while True:
        try:
            ...
            # Controlla se la risoluzione corrisponde a 1920x1080 e cerca il refresh rate più alto
            if resolution == (target_width, target_height) and refresh_rate > max_refresh_rate:
...
if __name__ == "__main__":
    ...
    # Cerca la risoluzione 1920x1080 con il refresh rate più alto
    target_width = 1920
    target_height = 1080
    best_mode = get_highest_refresh_rate_for_resolution(target_width, target_height)
    
    if best_mode:
        print(f"Imposto la risoluzione: {best_mode.PelsWidth}x{best_mode.PelsHeight}, Frequenza: {best_mode.DisplayFrequency}Hz")
        change_display_settings_with_qres(qres_executable, best_mode.PelsWidth, best_mode.PelsHeight, best_mode.DisplayFrequency)
    else:
        print("Risoluzione 1920x1080 non trovata.")
